...
window.inicializarVista = function() {
    ...
    const tabla = tablaElement.DataTable({
        ...
        "columns": [
            { "data": "codigo" }, { "data": "tema" }, { "data": "lugar" }, { "data": "fecha" },
            { "data": "firma", "render": function(data) { const estado = String(data).toLowerCase(); let badgeClass = 'badge-info'; if (estado === 'finalizado') badgeClass = 'badge-success'; else if (estado === 'borrador') badgeClass = 'badge-secondary'; return `<span class="badge ${badgeClass}">${data}</span>`; } },
            { "data": null, "defaultContent": ` <button class="btn btn-xs btn-primary btn-editar-encabezado" title="Editar Encabezado"><i class="fas fa-pencil-alt"></i></button> <button class="btn btn-xs btn-success btn-gestionar-contenido" title="Gestionar Contenido"><i class="fas fa-stream"></i></button> <button class="btn btn-xs btn-warning btn-generar-qr" title="Compartir (QR)"><i class="fas fa-qrcode"></i></button> <button class="btn btn-xs btn-info btn-finalizar-acta" title="Finalizar Acta"><i class="fas fa-check-circle"></i></button> <button class="btn btn-xs btn-danger btn-generar-pdf" title="Generar PDF"><i class="fas fa-file-pdf"></i></button> `, "orderable": false }
        ],
        "order": [[ 3, "desc" ]],
    ...
    $('#tabla-actas tbody').on('click', 'button', async function() {
        ...
        if ($(this).hasClass('btn-editar-encabezado')) {
            ...
            if (confirm(`¿Estás seguro de que deseas finalizar el acta ${data.codigo}?`)) {
                ...
                .catch(error => { window.mostrarNotificacion(`No se pudo finalizar el acta. ${error.message}`, 'danger'); });
            }
        }
        ...
        // --- Lógica para el botón de GENERAR PDF ---
            ...
            try {
                const colorPrincipal = '#2c3e50'; // Azul oscuro de ejemplo
                const colorSecundario = '#ff7300'; // Naranja oscuro que definiste
                ...
                doc.autoTable({
                    ...
                    head: [
                        // Fila 1: Le aplicamos el colorPrincipal
                        [{ 
                            content: '2. Desarrollo', 
                            colSpan: 3, 
                            styles: { fontStyle: 'bold', fillColor: colorPrincipal, textColor: 255 } 
                        }],
                        // Fila 2: Convertimos cada título en un objeto para aplicarle el colorSecundario
                        [
                    ...
                    body: desarrolloBody,
                    theme: 'grid',
                    // CORRECCIÓN: Se añade 'headStyles' para asegurar que todas las filas del encabezado tengan el color correcto.
                    headStyles: { fontStyle: 'bold', fillColor: colorPrincipal, textColor: 255 }
                });

                // --- 5. Compromisos ---
                const compromisosBody = pdfData.contenido.filter(item => item.compromisos && item.compromisos.trim() !== '').map(item => {
                ...
                if (compromisosBody.length > 0) {
                    doc.autoTable({
                        startY: doc.lastAutoTable.finalY + 10,
                        head: [[{ content: '3. Compromisos', colSpan: 3, styles: { fontStyle: 'bold' } }], 
                            [
                        ...
                        theme: 'grid',
                        // CORRECCIÓN: Se añade 'headStyles' para el color.
                        headStyles: { fontStyle: 'bold', fillColor: colorPrincipal, textColor: 255 }
                    });
                }
                ...
                if (pdfData.firmas && pdfData.firmas.length > 0) {
                    ...
                    doc.autoTable({
                        startY: doc.lastAutoTable.finalY + 10,
                        head: [[{ content: '4. Listado de Asistencia', colSpan: 4, styles: { fontStyle: 'bold' } }], 
                        ...
                        body: firmasBody,
                        theme: 'grid',
                        // CORRECCIÓN: Se usa 'headStyles' con tu color secundario.
                        headStyles: { fontStyle: 'bold', fillColor: colorSecundario, textColor: 255 , fillColor: colorPrincipal,},
                        columnStyles: {
            // Las columnas se numeran desde 0.
            // Los valores están en milímetros (el ancho de una hoja A4 es 210mm).
            
            // Columna 0: 'Nombre Completo'. 'auto' para que ocupe el espacio sobrante.
            0: { cellWidth: 'auto' }, 
            
            // Columna 1: 'Cargo'. Le damos un ancho fijo y pequeño.
            1: { cellWidth: 35 }, 
            
            // Columna 2: 'Empresa'. Ancho intermedio.
            2: { cellWidth: 40 }, 
            
            // Columna 3: 'Firma'. Le damos un ancho fijo y más grande.
        ...
        didDrawCell: function(data) {
            if (data.column.index === 3 && data.cell.section === 'body') {
                ...
                if (firmaBase64) {
                    // La lógica dinámica que creamos seguirá funcionando perfectamente.
